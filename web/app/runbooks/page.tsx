    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">{runbooks.length} runbooks</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {runbooks.map((rb) => (
          <div key={rb.ID} className="bg-gray-900 border border-gray-800 rounded-lg p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3">
                  <h2 className="text-base font-semibold text-white">{rb.Name}</h2>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Updated: {fmtTime(rb.UpdatedAt)} · ID: <span className="font-mono">{rb.ID}</span>
                </p>
              </div>
            </div>
            ...
            <pre className="mt-3 text-xs text-gray-400 bg-gray-950 rounded p-3 overflow-hidden max-h-24 whitespace-pre-wrap">
              {rb.Content?.slice(0, 300)}{rb.Content?.length > 300 ? "…" : ""}
            </pre>
          </div>
        ))}
      </div>
